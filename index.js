    ...
    (function() {
        ...
        function resolveClientReference(bundlerConfig, metadata) {
            if (bundlerConfig) {
                ...
                const resolvedModuleData = moduleExports[metadata[NAME]];
                name;
        ...
        function dispatchHint(code, model) {
            ...
            if (dispatcher) {
                ...
                case "C":
                    {
                        const _refined = refineModel(code, model);
                        if (typeof _refined === "string") {
                            const _href = _refined;
                            dispatcher.preconnect(_href);
                        } else {
                            const _href2 = _refined[0];
                            const crossOrigin = _refined[1];
                ...
                case "L":
                    {
                        const _refined2 = refineModel(code, model);
                        const _href3 = _refined2[0];
                        const as = _refined2[1];
                        if (_refined2.length === 3) {
                            const options = _refined2[2];
                ...
                case "m":
                    {
                        const _refined3 = refineModel(code, model);
                        if (typeof _refined3 === "string") {
                            const _href4 = _refined3;
                            dispatcher.preloadModule(_href4);
                        } else {
                            const _href5 = _refined3[0];
                            const _options = _refined3[1];
                ...
                case "S":
                    {
                        const _refined4 = refineModel(code, model);
                        if (typeof _refined4 === "string") {
                            const _href6 = _refined4;
                            dispatcher.preinitStyle(_href6);
                        } else {
                            const _href7 = _refined4[0];
                            const precedence = _refined4[1] === 0 ? undefined : _refined4[1];
                            const _options2 = _refined4.length === 3 ? _refined4[2] : undefined;
                ...
                case "X":
                    {
                        const _refined5 = refineModel(code, model);
                        if (typeof _refined5 === "string") {
                            const _href8 = _refined5;
                            dispatcher.preinitScript(_href8);
                        } else {
                            const _href9 = _refined5[0];
                            const _options3 = _refined5[1];
                ...
                case "M":
                    {
                        const _refined6 = refineModel(code, model);
                        if (typeof _refined6 === "string") {
                            const _href10 = _refined6;
                            dispatcher.preinitModuleScript(_href10);
                        } else {
                            const _href11 = _refined6[0];
                            const _options4 = _refined6[1];
        ...
        function error(format) {
            {
                {
                    for (const _len2 = arguments.length, args = new Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
        ...
        function describeObjectForErrorMessage(objectOrArray, expandedName) {
            ...
            if (isArray(objectOrArray)) {
                if (jsxChildrenParents.has(objectOrArray)) {
                    ...
                    for (const _i = 0; _i < _array.length; _i++) {
                        if (_i > 0) {
                            str += ", ";
                        }
                        const _value = _array[_i];
                        const _substr = void 0;
                ...
                if (objectOrArray.$$typeof === REACT_ELEMENT_TYPE) {
                    ...
                    for (const _i2 = 0; _i2 < names.length; _i2++) {
                        str += " ";
                        const name = names[_i2];
                        str += describeKeyForErrorMessage(name) + "=";
                        const _value2 = object[name];
                        const _substr2 = void 0;
                    ...
                    for (const _i3 = 0; _i3 < _names.length; _i3++) {
                        if (_i3 > 0) {
                            str += ", ";
                        }
            ...
            function resolveToJSON(key, value) {
                ...
                if (typeof value === "object") {
                    ...
                    if (value instanceof Set) {
                        const _partJSON = JSON.stringify(Array.from(value), resolveToJSON);
                        if (formData === null) {
                            formData = new FormData();
                        }
                ...
                if (typeof value === "string") {
                    ...
                    if (value[value.length - 1] === "Z") {
                        ...
                        // $FlowFixMe[incompatible-use]
                        const _originalValue = parent[key];
        ...
        const initializingChunk = null;
        const initializingChunkBlockedModel = null;
        ...
        function parseModelString(response, parentObject, key, value) {
            if (value[0] === "$") {
                ...
                case "@":
                    {
                        // Promise
                        const _id = parseInt(value.slice(2), 16);
                        const _chunk = getChunk(response, _id);
                        return _chunk;
                    }
                case "S":
                    {
                ...
                case "F":
                    {
                        // Server Reference
                        const _id2 = parseInt(value.slice(2), 16);
                        const metadata = getOutlinedModel(response, _id2);
                        return createServerReferenceProxy(response, metadata);
                    }
                case "Q":
                    {
                        // Map
                        const _id3 = parseInt(value.slice(2), 16);
                        const data = getOutlinedModel(response, _id3);
                        return new Map(data);
                    }
                case "W":
                    {
                        // Set
                        const _id4 = parseInt(value.slice(2), 16);
                        const _data = getOutlinedModel(response, _id4);
                ...
                default:
                    {
                        ...
                        const _id5 = parseInt(value.slice(1), 16);
                        const _chunk2 = getChunk(response, _id5);
        ...
        function resolveErrorDev(response, id, digest, message, stack) {
            const error = new Error(message || "An error occurred in the Server Components render but no message was provided");
            error.stack = stack;
            error.digest = digest;
        ...
        function processBinaryChunk(response, chunk) {
            const i = 0;
            const rowState = response._rowState;
            const rowID = response._rowID;
            const rowTag = response._rowTag;
            ...
            while (i < chunkLength) {
                ...
                if (lastIdx > -1) {
                    ...
                    const _length = chunk.byteLength - i;
                    const remainingSlice = new Uint8Array(chunk.buffer,offset,_length);
